// Fill out your copyright notice in the Description page of Project Settings.

// Parent Header
#include "Subsystem/ItemInstanceSubsystem.h"

// Engine Headers
#include "Manager/RAssetManager.h"

// Project Headers
#include "Definition/InventoryPrimaryAsset.h"
#include "Item/ItemInstance.h"
#include "LatentDelegates.h"
#include "Log/LogCategory.h"
#include "Log/LogMacro.h"
#include "Subsystem/InventorySubsystem.h"




UItemInstance* UItemInstanceSubsystem::GetItem(const FGuid& InventoryId, const FPrimaryAssetId& AssetId)
{
	FItemInstanceId InstanceId(InventoryId, AssetId);
	TObjectPtr<UItemInstance>* FoundItem = ItemInstances.Find(InstanceId);
	if (FoundItem)
	{
		return FoundItem->Get();
	}

	return CreateItem_Internal(InventoryId, AssetId);
}

bool UItemInstanceSubsystem::RemoveItem(const FGuid& InventoryId, const FPrimaryAssetId& AssetId)
{
	FItemInstanceId InstanceId(InventoryId, AssetId);
	TObjectPtr<UItemInstance>* FoundItem = ItemInstances.Find(InstanceId);
	if (!FoundItem)
	{
		return false;
	}

	UInventorySubsystem* InventorySubsystem = WeakInventory.Get();
	UItemInstance* Item = FoundItem->Get();
	bool bResult = CommitItem_Internal(Item, InventorySubsystem);
	if (!bResult)
	{
		return false;
	}

	ItemInstances.Remove(InstanceId);

	Item->ResetItem();
	Item->MarkAsGarbage();

	return true;
}

void UItemInstanceSubsystem::CommitAllItems()
{
	UInventorySubsystem* InventorySubsystem = WeakInventory.Get();
	for (const TPair<FItemInstanceId, UItemInstance*>& Pair : ItemInstances)
	{
		CommitItem_Internal(Pair.Value, InventorySubsystem);
	}
}

bool UItemInstanceSubsystem::CommitItem(const FGuid& InventoryId, const FPrimaryAssetId& AssetId)
{
	FItemInstanceId InstanceId(InventoryId, AssetId);
	TObjectPtr<UItemInstance>* FoundItem = ItemInstances.Find(InstanceId);
	if (!FoundItem)
	{
		return false;
	}

	UInventorySubsystem* InventorySubsystem = WeakInventory.Get();
	UItemInstance* Item = FoundItem->Get();

	bool bResult = CommitItem_Internal(Item, InventorySubsystem);
	return bResult;
}


UItemInstance* UItemInstanceSubsystem::CreateItem_Internal(const FGuid& InventoryId, const FPrimaryAssetId& AssetId)
{
	UInventorySubsystem* InventorySubsystem = WeakInventory.Get();
	if (!IsValid(AssetManager) || !IsValid(InventorySubsystem))
	{
		return nullptr;
	}

	FAssetData AssetData;
	AssetManager->GetPrimaryAssetData(AssetId, AssetData);

	bool bPersistWhenEmpty = false;
	InventoryPrimaryAsset::GetPersistWhenEmpty(AssetData, bPersistWhenEmpty);

	bool bStackable = false;
	InventoryPrimaryAsset::GetStackable(AssetData, bStackable);

	if (!bPersistWhenEmpty || !bStackable)
	{
		LOG_ERROR(LogItemInstance, TEXT("Item instance cannot be created for non-persistent item or non-stackable item"));
		return nullptr;
	}

	const FInventoryItem* Item = InventorySubsystem->GetItem(InventoryId, AssetId);
	if (!Item)
	{
		LOG_ERROR(LogItemInstance, TEXT("Unable to find item"));
		return nullptr;
	}

	UItemInstance* NewInstance = NewObject<UItemInstance>();
	if (!IsValid(NewInstance))
	{
		LOG_ERROR(LogItemInstance, TEXT("Failed to create item instance"));
		return nullptr;
	}

	NewInstance->InitializeItem(InventoryId, AssetId, (*Item));

	FItemInstanceId InstanceId(InventoryId, AssetId);
	ItemInstances.Add(InstanceId, NewInstance);

	return NewInstance;
}

bool UItemInstanceSubsystem::CommitItem_Internal(UItemInstance* Item, UInventorySubsystem* InventorySubsystem)
{
	if (!IsValid(Item) || !IsValid(InventorySubsystem))
	{
		return false;
	}

	FGuid InventoryId = Item->GetInventoryId();
	FPrimaryAssetId AssetId = Item->GetAssetId();
	FGuid ItemId = Item->GetItemId();

	int Quantity = Item->GetQuantity();

	bool bResult = InventorySubsystem->UpdateItemById(InventoryId, AssetId, ItemId,
		[Quantity](FInventoryItem* Item)
		{
			if (Item)
			{
				Item->Quantity = Quantity;
				Item->Sanitize();
			}
		}
	);

	return bResult;
}


void UItemInstanceSubsystem::OnPreGameInitialized()
{
	FLatentDelegates::OnPreGameInitialized.RemoveAll(this);

	UInventorySubsystem* Inventory = UInventorySubsystem::Get(GetGameInstance());
	WeakInventory = TWeakObjectPtr<UInventorySubsystem>(Inventory);

	AssetManager = URAssetManager::GetIfInitialized();
}

bool UItemInstanceSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
	return true;
}

void UItemInstanceSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);

	FLatentDelegates::OnPreGameInitialized.AddUObject(this, &UItemInstanceSubsystem::OnPreGameInitialized);

	LOG_WARNING(LogItemInstance, TEXT("ItemInstanceSubsystem initialized"));
}

void UItemInstanceSubsystem::Deinitialize()
{
	FLatentDelegates::OnPreGameInitialized.RemoveAll(this);

	CommitAllItems();
	ItemInstances.Empty();

	AssetManager = nullptr;
	WeakInventory.Reset();

	LOG_WARNING(LogItemInstance, TEXT("ItemInstanceSubsystem deinitialized"));
	Super::Deinitialize();
}


UItemInstanceSubsystem* UItemInstanceSubsystem::Get(UWorld* World)
{
	if (!IsValid(World))
	{
		return nullptr;
	}
	return Get(World->GetGameInstance());
}

UItemInstanceSubsystem* UItemInstanceSubsystem::Get(UGameInstance* GameInstance)
{
	if (!IsValid(GameInstance))
	{
		return nullptr;
	}
	return GameInstance->GetSubsystem<UItemInstanceSubsystem>();
}

